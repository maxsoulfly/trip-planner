...
### 2026-07-07 — Step 32: Editable place types and traits (moved to IndexedDB)

- **Done:**
  - `src/db/db.js` — `version(5)` adds `placeTypes: 'key, order'` and `venueTraits: 'key, order'` tables (empty on upgrade, seeded on mount). `order` had to be indexed, not just the `key` PK — `getAllPlaceTypes()`/`getAllVenueTraits()` use `orderBy('order')`, and Dexie throws `SchemaError: KeyPath order on object store ... is not indexed` if it isn't declared in the schema string. Caught this via a Playwright smoke test (blank white screen, `SettingsProvider` stuck at `loaded: false`) before it shipped.
  - `src/db/repo.js` — `PLACE_TYPES_SEED`/`VENUE_TRAITS_SEED` (15 types, 9 traits — consolidates keyword lists that used to live separately in `PlaceForm.jsx` TYPE_KEYWORDS and `blobParser.js` TRAIT_HINTS); `seedTypesAndTraits()` (count-gated, no-op after first run); `getAllPlaceTypes`, `getAllVenueTraits`, `putPlaceType`, `putVenueTrait`, `deletePlaceType` (blocks if any place still uses the key), `deleteVenueTrait` (unconditional — trait tags live in the free-form `tags` array, not an indexed field, so there's no cheap usage count to gate on).
  - `src/context/SettingsContext.jsx` — new. `SettingsProvider` loads `placeTypes`/`venueTraits` on mount, exposes `{ placeTypes, venueTraits, loaded, reload }`; renders `null` until `loaded` so no component ever sees an empty vocabulary flash. `useSettings()` throws if called outside the provider. Also exports `typeMetaFrom(placeTypes, key)` — the shared replacement for the old static `typeMeta()` lookup, with fallback to the `other` type then a bare pin so a stale/deleted key never crashes render.
  - `src/main.jsx` — wrapped render in an async IIFE; `await seedTypesAndTraits()` runs before `createRoot(...).render()`; `<SettingsProvider>` wraps `<App />`.
  - Swapped `PLACE_TYPES`/`VENUE_TRAITS`/`typeMeta` imports from `constants.js` for `useSettings()` in: `App.jsx`, `PlaceForm.jsx`, `PlaceCard.jsx`, `PlaceList.jsx`, `PlacePicker.jsx`, `TripGrid.jsx`, `BlobPreview.jsx`, `SlotCell.jsx`, `CsvImport.jsx` — nine consumers found via grep, one more than the brief's estimate (`SlotCell.jsx` and `CsvImport.jsx` weren't called out but both referenced the old constants directly). `BLOCKS`, `STATUSES`, `WEEKDAYS`, `blockMeta` stayed static imports from `constants.js`, untouched, per the brief.
  - `PlaceForm.jsx` — module-level `TYPE_KEYWORDS`/`detectType` removed; `detectType` is now a closure inside the component over `placeTypes` from context. **Deviation from the brief's literal pseudocode:** the brief's version returns the first type (in `placeTypes` array order) with any keyword match — but DB order is now the *display* order (taproom before bottle_shop), not a specificity order, so "ABC Beer Shop" would wrongly detect as `taproom` (matches generic "beer") before ever checking `bottle_shop`'s more specific "beer shop". Rewrote it to pick the **longest matching keyword** across all types instead of the first type in list order, reproducing the original ordered-list's intent ("more-specific phrases win") without depending on display order. Also builds `traitHints` via `useMemo` from `venueTraits` (`{ keyword: [traitKey,...] }`, grouped so one keyword can suggest multiple traits, e.g. "bottle shop" → both `bottles-to-go` and `craft-beer`) and passes it to `parseBlob(text, traitHints)`.
  - `src/utils/blobParser.js` — removed the hardcoded `TRAIT_HINTS` constant; `parseBlob(text, traitHints = {})` now takes it as a parameter. Stays a pure function — no React/Dexie import added.
  - `src/utils/exportHtml.js` — pure module, can't call `useSettings()`. Removed the `PLACE_TYPES` import and module-level `TYPE_EMOJI`; `generateDaySheet(trip, scheduleItems, placesMap, placeTypes = [])` now builds `typeEmoji` internally and threads it as a parameter through `renderDay` → `renderBlock` → `renderPlace` (3 signatures touched). `TripGrid.jsx`'s `handleExportHtml` now calls `generateDaySheet(trip, scheduleItems, places, placeTypes)` with `placeTypes` from context.
  - `src/components/CsvImport.jsx` — `rowToPlace(row, mapping)` → `rowToPlace(row, mapping, placeTypes)`; `PLACE_TYPE_KEYS` module constant removed, computed inline from the passed `placeTypes` instead. `STATUSES`/`STATUS_KEYS` untouched (status vocabulary wasn't in scope).
  - `src/components/TypesTraitsModal.jsx` + `.css` — new. Two tabs (PLACE TYPES / TRAITS) over `useSettings()`'s `placeTypes`/`venueTraits`; place counts via `useMemo` over the `places` prop (`p.type === key` for types, `(p.tags||[]).includes(key)` for traits — tags is already an array per the Place schema, so this is an exact count, not the "approximate" one the brief anticipated). Inline expand-in-place EDIT panel (emoji/label/key-permanent/keywords), SAVE/CANCEL/DELETE (DELETE only rendered when `placeCount === 0`), and a `+ ADD TYPE`/`+ ADD TRAIT` inline form with `uniqueKey()` collision-checked key generation (loops `_2`, `_3`, ... rather than the brief's single `_2` fallback, to guarantee uniqueness). Every mutation calls `reload()` from context (so the change is live everywhere immediately) then the `onRefresh` prop.
  - `src/components/AdminModal.jsx` — renamed the "City Management" section to "Vocabulary"; added a second row with a `◈ TYPES & TRAITS` button and `showTypesTraits` state, rendering `<TypesTraitsModal places={allPlaces} onClose={...} onRefresh={onRefresh} />` alongside the existing `CitiesModal`.
- **Deviations:**
  - `isIncomplete(p)` in `App.jsx` was **not** changed — the brief assumed it read from `PLACE_TYPES`, but it only ever checked hardcoded permanent key strings (`p.type === 'other'`, a hardcoded `hoursTypes` list of keys). Since keys are permanent (Option C), this check is already correct against the new editable vocabulary and needed no update.
  - `detectType` keyword-matching precision fix (see above) — not in the brief, found while implementing it.
  - Found and converted two extra consumers (`SlotCell.jsx`, `CsvImport.jsx`) beyond the brief's explicit list, via a grep sweep for `PLACE_TYPES|VENUE_TRAITS|typeMeta` across `src/`.
  - Caught and fixed a schema bug (`order` not indexed) that blanked the entire app — not something the brief could have anticipated from the pseudocode alone.
- **Schema/contract changes:** `db.js` version(5) — new `placeTypes`/`venueTraits` tables (see above). No changes to `places`/`trips`/`scheduleItems` shape or indexes. `constants.js` `PLACE_TYPES`/`VENUE_TRAITS`/`typeMeta`/`PLACE_TYPE_MAP` left in place, unused by any component now (kept for reference per the brief — `BLOCKS`/`STATUSES`/`WEEKDAYS`/`blockMeta` are still the live exports other code imports).
- **Verification:** Ran the app in headless Chromium via Playwright (installed ad-hoc for this session — not a project dependency). Confirmed: no console errors on load; TYPE/TRAIT filter selects populate (16/10 options incl. "ALL"); Admin → ◈ TYPES & TRAITS opens with 15 type rows; EDIT → inline panel with emoji/label/key(permanent)/keywords; renamed "Taproom" → "Taproom TEST", SAVE, row updated immediately, and the new label appeared instantly in a freshly-opened Add Place TYPE dropdown (`🍺 Taproom TEST` first) — proving the context reload propagates without a page refresh. Also re-verified the blob QUICK PASTE trait-suggestion flow end to end with a `taproom` category line → suggested `🍺 Craft Beer` / `🚰 Taps On-Site` chips, confirming the `traitHints` refactor (constant → parameter) didn't regress. Did not verify the DELETE-button place-count gating in-browser (fresh DB has 0 places, so DELETE always shows) — reviewed by code inspection only.
- **Known issues / TODO:** `deleteVenueTrait` doesn't check trait usage before deleting (see repo.js comment) — an orphaned tag string can be left on places if a trait in active use is deleted via the modal's DELETE button (which is only gated on the same `p.tags.includes(key)` count check client-side, so this is low-risk in practice but not enforced at the repo layer like `deletePlaceType` is).
- **Next:** TBD.

---

### 2026-07-07 — Step 33: Split hours (two windows per day, e.g. lunch/dinner)

- **Done:**
  - `src/utils/hoursParser.js` — extracted the existing dash-splitting/PM-inference range parser out of `parseHoursValue` into a new `parseSingleRange(s)` helper (identical logic, just factored out so it can be reused — nothing about single-range parsing changed). `parseHoursValue` gained a pre-check: if the value string contains a comma, splits into exactly two parts and runs each through `parseSingleRange`; if both parse, returns `{ open, close, open2, close2 }` instead of falling through to the single-range path. Covers Format B (`Tuesday: 12–3 pm, 4–9 pm`) automatically since it already calls `parseHoursValue` on the full value string. Format A's alternating day/value loop now also looks one line past the hours line: if that line isn't a weekday name and parses as its own plain range, it's folded in as `open2`/`close2` and the loop advances 3 lines instead of 2 (guarded so `Closed`/24h/already-split lines never trigger it).
  - `src/components/PlaceForm.jsx` — `hours-editor` per-day row gains a second `hours-times` block (open2/close2 `<input type="time">`, ✕ remove) shown when `h.open2 !== undefined`; otherwise a dashed `+ split` button appears (open days only) that calls new `addSplitWindow(key)` (sets `open2`/`close2` to `''`). New `removeSplitWindow(key)` strips both fields back out via destructure-omit. `setDayTime` needed no change — it was already generic on the field name, so `setDayTime(key, 'open2', ...)` works as-is. New module-level `sanitizeHours(hours)` drops `open2`/`close2` from the saved record when they're still empty strings (window added but not filled in); wired into `handleSubmit`'s `openingHours` field.
  - `src/components/PlaceForm.css` — `.hours-times--split` (tighter left margin than the first window's `margin-left: auto`), `.hours-split-add` (dashed steel pill, amber on hover), `.hours-split-remove` (bare ✕, rust on hover).
  - `src/components/PlaceCard.jsx` `getStatusBadge` — first-window open/closed check unchanged; if not open and `entry.open2`/`close2` are set, runs the identical overnight-aware check against the second window and adopts it if true. Factored the repeated open/close-to-minutes math into `openMinsOf`/`closeMinsOf` helpers (used for both windows) rather than duplicating the 4-line block inline. **Went beyond the brief's snippet:** the brief only showed reassigning `openMins`/`closeMins` when the second window is currently open (which turns out to be inert dead code, since the function returns `OPEN` immediately after) — it didn't address the "between windows" or "first window already passed" cases for the OPENS SOON / OPENS HH:MM branch. Rewrote that branch to collect *both* windows' opening times, filter to the ones still ahead of now, and pick the soonest — verified via an ad-hoc script that `15:30` with hours `12–15, 16–21` correctly returns "OPENS 16:00" instead of falling through to "CLOSED TODAY" (which is what a literal implementation of the brief's snippet would have produced).
  - `src/utils/exportHtml.js` `hoursSpan` — after getting the base string from `hoursForDay` (unchanged, still lives in `hours.js`), looks up the raw day entry directly and appends `· HH:MM–HH:MM` when `open2`/`close2` are present. `hours.js` itself was **not** touched — the brief only listed `exportHtml.js`, and `hoursForDay`'s other caller (`PlaceCard.jsx`'s day-pip tooltip) was out of scope, so changing the shared helper would've silently changed that tooltip too.
- **Deviations:**
  - `getStatusBadge`'s OPENS SOON/OPENS logic rewritten beyond the brief's literal snippet (see above) — necessary for correctness once a first window has already closed for the day.
  - `parseSingleRange` extraction in `hoursParser.js` — the brief said the existing single-range logic "stays untouched"; it does, byte-for-byte, just moved into its own function so the comma pre-check can call it without duplicating ~15 lines of PM-inference logic.
- **Schema/contract changes:** None — `open2`/`close2` are plain optional keys on the existing per-day `openingHours[key]` object, same contract as before (absent = unknown, `null` = closed). No `db.js` version bump, no `repo.js` change, matching the brief.
- **Verification:** Ran an ad-hoc Node script (ESM dynamic import, no test framework in this repo) against `parseGoogleHours` covering both formats × split/unaffected-plain/closed — all 8 cases matched expected output. Ran a second ad-hoc script re-implementing `getStatusBadge`'s pure core to check all 7 time-of-day cases against a `12–15, 16–21` split-hours entry (before/during/between/soon-before/during-second/after-both), plus a no-second-window control — all correct. Then drove the actual app in headless Chromium via Playwright: added a place, opened Monday, clicked `+ split`, filled all four time inputs, saved, and reopened the edit form — all four values (`12:00`/`15:00`/`16:00`/`21:00`) persisted correctly through the save/reload round-trip with zero console errors.
- **Known issues / TODO:** None.
- **Next:** TBD.
